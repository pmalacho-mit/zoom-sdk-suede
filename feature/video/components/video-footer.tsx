...
import { useSearchParams } from "../../../react-router.stub";
...
const VideoFooter = (props: VideoFooterProps) => {
  ...
  const [activeVideoProcessor, setActiveVideoProcessor] = useState<
    Processor | undefined
  >();
  const [activeAudioProcessorList, setActiveAudioProcessorList] = useState<
    Processor[]
  >([]);
  ...
  const [broadcastStreamStatus, setBroadcastStreamStatus] = useState<
    "" | BroadcastStreamingStatus
  ...
  const [isVideoMirrored, setIsVideoMirrored] = useState<boolean>(
    !!mediaStream?.isVideoMirrored()
  );
  ...
  const onMicrophoneMenuClick = async (key: string) => {
    if (mediaStream) {
      ...
      if (type === "microphone") {
        ...
        const processor = audioProcessorList.find(
          (item) => item.name === processorName
        );
        if (processor && mediaStream) {
          try {
            ...
            let index = activeAudioProcessorList.findIndex(
              (p) => p.name === processorName
            );
            if (index > -1) {
              await mediaStream?.removeProcessor(tempProcessor);
              setActiveAudioProcessorList(
                activeAudioProcessorList.filter((p) => p.name !== processorName)
              );
            } else {
  ...
  const onScreenShareClick = useCallback(async () => {
    if (mediaStream?.getShareStatus() === ShareStatus.End && selfShareCanvas) {
      await mediaStream?.startShareScreen(selfShareCanvas, {
        ...
        simultaneousShareView:
          searchParams.get("simultaneousShareView") === "1",
      });
    }
  ...
  const updateWatermarkImage = useCallback(
    (processor: Processor, imageUrl: string) => {
  ...
  const onVideoProcessorClick = useCallback(async (processor: any) => {
    let tempProcessor = createdProcessorList?.find(
    ...
    if (!tempProcessor) {
      try {
        ...
        setCreatedProcessorList([
          ...createdProcessorList,
          tempProcessor as Processor,
        ]);
      } catch (e) {
        console.log(e);
      }
  ...
  const onVideoScreenshotTaken = useCallback((payload: any) => {
    ...
    message.info(
      `${displayName}(User:${userId}) just took a screenshot of your video`
    );
  }, []);
  const onShareViewScreenshotTaken = useCallback((payload: any) => {
    const { displayName, userId } = payload;
    message.info(
      `${displayName}(User:${userId}) just took a screenshot of your sharing`
    );
  }, []);
  useEffect(() => {
